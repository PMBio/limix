...
class ProdCov(ACombinatorCov):
    """
    Element wise product of multiple covariance matrices.
    The number of paramteters is the sum of the parameters of the single covariances.
    """
    ...
    def K_grad_i(self,i):
        ...
        for j in range(len(self.covars)):
            ...
            if (i < istop):
                idx = i - istart
                R = self.getCovariance(j).K_grad_i(idx)
                for k in range(len(self.covars)):
                    if k == j: continue
                    R *= self.getCovariance(k).K()
                return R
            istart = istop
        return None
    ...
    def K_hess_i_j(self, i, j):
        ...
        if c1==c2:
            r = self.getCovariance(c1).K_hess_i_j(i0, j0)
            for c3 in range(len(self.covars)):
                if c3 == c1: continue
                r *= self.getCovariance(c3).K()
        else:
            r = self.getCovariance(c1).K_grad_i(i0) * self.getCovariance(c2).K_grad_i(j0)
            for c3 in range(len(self.covars)):
                if c3 == c1: continue
                if c3 == c2: continue
                r *= self.getCovariance(c3).K()
